  ...
  const fetchAvailableSlots = async () => {
    ...
    const availableEmployees = selectedEmployee 
      ...
      : employees.filter(emp => 
          emp.employee_services.some(es => es.service_id === selectedService.id)
        );
    ...
    const { data: existingReservations } = await supabase
      ...
      .in('employee_id', availableEmployees.map(emp => emp.id));
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-muted-foreground">Proceso fácil y rápido en 4 pasos</p>
      </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                {availableSlots.map((slot, index) => (
                  <Button
                    key={index}
                    variant={selectedSlot?.start_time === slot.start_time && selectedSlot?.employee_id === slot.employee_id ? "default" : "outline"}
