...
export const ServicesSection = () => {
  ...
  return <section id="services" className="py-16 sm:py-24 bg-gradient-to-b from-background to-muted/20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          return <ServiceCard key={service.id} id={service.id} name={service.name} description={service.description} originalPrice={service.original_price_cents} finalPrice={service.final_price_cents} savings={service.savings_cents} duration={service.duration_minutes} imageUrl={service.image_url} type={service.type} discountType={service.savings_cents > 0 ? service.appliedDiscount?.discount_type : undefined} discountValue={service.savings_cents > 0 ? service.appliedDiscount?.discount_value : undefined} comboServices={comboServices} onSelect={() => navigate(`/book?service=${service.id}&estilista=cualquier&step=2`)} variant="landing" showExpandable={true} />;
        })}
          </div> :
      ...
      <Carousel className="w-full">
            ...
            return <CarouselItem key={service.id} className="pl-2 md:pl-4 basis-full sm:basis-1/2 md:basis-1/2 lg:basis-1/3">
