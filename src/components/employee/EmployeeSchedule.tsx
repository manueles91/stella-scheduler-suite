  ...
  const fetchSchedules = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('employee_schedules').select('*').eq('employee_id', effectiveEmployeeId).order('day_of_week');
      if (error) {
    ...
    try {
      if (schedule.id) {
        ...
        if (!schedule.is_available) {
          ...
          const {
            error
          } = await supabase.from('employee_schedules').delete().eq('id', schedule.id);
