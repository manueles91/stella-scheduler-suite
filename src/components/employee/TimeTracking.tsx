...
export const TimeTracking = ({
  ...
  const fetchClients = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('profiles').select('id, full_name, email').eq('role', 'client').order('full_name');
      if (error) throw error;
      setClients(data || []);
    } catch (error) {
  ...
  const renderTimeSlots = () => {
    const slots = [];
    for (let hour = 6; hour <= 22; hour++) {
      slots.push(<div key={hour} className="relative border-b border-gray-200" style={{
    ...
    return <div className="flex flex-col h-screen bg-gray-50">
        ...
        <div className="bg-white border-b px-4 py-2">
          <div className="flex gap-1">
            {weekDays.map(day => <Button key={day.toISOString()} variant={isSameDay(day, selectedDate) ? "default" : "ghost"} size="sm" onClick={() => setSelectedDate(day)} className="flex-1 flex flex-col items-center py-2 h-auto">
