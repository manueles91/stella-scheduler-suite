-- Drop existing views and recreate without security definer issues
DROP VIEW IF EXISTS admin_reservations_view;
DROP VIEW IF EXISTS employee_calendar_view;

-- Create admin reservations view with proper access patterns
...
SELECT 
  ...
  COALESCE(r.final_price_cents, s.price_cents) AS service_price_cents,
...
-- Create RLS policies for the views that inherit from underlying table policies
-- Views will automatically respect the RLS policies of the underlying tables