-- Fix admin_reservations_view RLS policies and ensure proper data access
-- This migration ensures both TimeTracking and DashboardSummary can access the same data

-- First, drop and recreate the view with proper structure
DROP VIEW IF EXISTS admin_reservations_view CASCADE;

-- Create the updated admin_reservations_view
...
-- Note: Views inherit RLS from underlying tables, but we need to ensure the policies work correctly

-- Enable RLS on the view (though it inherits from underlying tables)
ALTER VIEW admin_reservations_view SET (security_invoker = true);

-- Add a comment explaining the view
COMMENT ON VIEW admin_reservations_view IS 'Combined view of individual reservations and combo reservations with proper RLS inheritance from underlying tables';
